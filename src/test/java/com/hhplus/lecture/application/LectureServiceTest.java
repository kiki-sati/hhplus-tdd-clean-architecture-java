...
class LectureServiceTest {
	...
	void 실패_강의신청_정원초과() {
		...
		lecture.getRegisteredUsers().addAll(
			IntStream.range(0, 30) // 30명의 등록자 생성
				.mapToObj(i -> (long)i) // 사용자 ID로 long형 변환
		...
		assertEquals(ErrorMessage.LECTURE_FULL.getMessage(), exception.getMessage()); // 직접 예외 메시지 확인
	}
	...
	void 실패_강의신청_중복신청() {
		...
		lecture.getRegisteredUsers().add(userId); // 강의에 사용자 추가
